...
# while index.html still receives a classic-script global that works on file://.
$normalizedBuilder = Join-Path (Get-Location) "tools\build-normalized-data.mjs"
if (Test-Path $normalizedBuilder) {
    & node $normalizedBuilder
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to generate data\normalized.data.js"
    }
}

# Generate the classified crafting inventory and its file:// browser wrapper.
$currencyIndexBuilder = Join-Path (Get-Location) "tools\build-currency-index.mjs"
if (Test-Path $currencyIndexBuilder) {
    & node $currencyIndexBuilder
    if ($LASTEXITCODE -ne 0) {
