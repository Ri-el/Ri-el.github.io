...
| Socket restrictions | `items.entries[].sockets` stores the only per-base socket number available in the export. The field is not self-describing; the runtime separately labels its later interpretation as an inferred maximum with zero fresh sockets. `methods.socketables.types` and `.limits` describe socketable types and global limits. |
...
| Desecrated modifiers | The Desecrate method and bone constraints are in `methods.crafting`. Its special modifier records use stable `AbyssMod...` keys and the `AbyssJewelMod` family convention, and appear in applicable `classmods` pools. All 32 retained Jewel records have required level 1 and all 193 equipment records have required level 65; these levels support Ancient Bones' level-40 floor. The export does not include human-readable reveal text or a complete Well-of-Souls selection procedure. |
...
| Sockets/Augments | `socketables.entries[]` contains Rune/Soul Core/Idol effects for martial, armour, caster, global, and class-specific cases. Keys in `entry.class` index `enums.classes` (item classes such as Wand or Body Armour); they are not `classes.entries[].id` modifier-pool IDs. This is exact effect/applicability data, not by itself a complete socket-state mutation procedure. |
...
- `crafting-items.json`: normalized crafting-method metadata, verified Omen actions, referenced crafting items, catalyst inventory, socketable effects/limits, and emotion mappings. New conversions also retain the socketable item-class enum map; the older checked-in snapshot can reconstruct the same 17 referenced enum names from `essences.json`, which uses that identical source enum.
...
`tools/build-normalized-data.mjs` keeps a complete `data/normalized.data.js` audit wrapper and also compiles `data/runtime.data.js` as `window.COE_RUNTIME_DATA`. The runtime projection contains every concrete base and simulator mapping, the exact implicit records those bases reference, the exact source modifier identities, weights, and tag conditions used by compiled modifier-pool overlays, and a compact `craftingMechanics` projection for Essences, socketables, socket limits, item-class applicability, and exact stat ranges. Full modifiers, crafting inventories, Essences, and provenance remain in repository-owned normalized JSON and the audit wrapper; they are not deleted merely to reduce browser startup. Both outputs remain classic scripts, preserving direct `file://` use without `fetch()`.
...
| Standard Essences | `essenceDisabledReason` | `CraftingEngine.applyEssence` | Exact item-class modifier IDs and groups are used for the source-backed Magic-to-Rare transition. Greater/Perfect Rare remove/add transitions remain explicitly labelled inferred. |
| Essence of the Breach | `essenceDisabledReason` | `CraftingEngine.applyEssence` | Uses the exact retained Amulet/Ring quality modifier; the Rare remove/add transition and full-prefix removal rule are explicitly inferred. |
| Gnawed, Preserved, and Ancient Abyssal Bones | `boneDisabledReason` | `startDesecrationFlow` / `CraftingEngine.startDesecration` | Reuses the existing Well flow with each retained direction, replacement count, reroll count, and item-class restriction; these shared transitions are labelled inferred. Ancient variants apply the retained level-40 floor to ordinary and Desecrated option pools. |
| Artificer's Orb | `socketDisabledReason` | `CraftingEngine.applyArtificerOrb` | Adds one empty Rune Socket up to the concrete base's explicitly inferred `socketCount` maximum. |
| Runes, Soul Cores, Idols, Abyssal Eyes, and Congealed Mist | `socketDisabledReason` | `CraftingEngine.applySocketable` | Inserts into an existing empty socket, resolves exact item-class/family effect data and ranges, enforces retained limits/corruption flags, and never replaces or removes contents. Talisman-only records remain blocked because no selectable Talisman base is retained. |
...
- The implementation uses exact normalized Essence modifier identities, groups, class mappings, socketable stat identities/ranges, limit records, and corruption flags. Any transition not stated by the source remains labelled `inferred`, including Greater/Perfect/Breach Rare remove/add rules and the socket-cap/default interpretation.
- No socket replacement or extraction operation is implemented because no retained method defines one. Three Talisman-only augments remain blocked because the normalized base catalogue has no selectable Talisman base.
- Missing modifier localization, absent exact Vaal probabilities, and incomplete state-transition rules continue to block Quality, Catalysts, Genesis, Delirium/Instilling, Runeforging, Sacrifice, and the remaining specifically identified records.
