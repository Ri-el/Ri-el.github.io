...
This is the Task 01 baseline. Concrete Amulet selection changes item identity and presentation; it does not change any crafting-item classification or mechanic.
...
Task 01 parity delta: **0 crafting-item statuses changed**.

This baseline deliberately does not claim that `currency-index.json` already satisfies the final per-entry parity contract. Task 03 must add authoritative applicability, handler, target-version evidence, verification, test reference, and blocker fields while generating the runtime inventory from the registry. Until that work and the later mechanic tasks are complete, the 489 `blocked_missing_data` records and the `probability_unverified` record remain explicit blockers.
