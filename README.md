...
| `data/desecrated-mods.json` | Source display pools for the Desecrate/Abyssal Bone feature. Legacy localized templates are split by Jewel/equipment pool; 0.5.4-targeted normalized metadata supplies required modifier levels 1 and 65 so Ancient Bones can enforce their level-40 floor. Hand-editable. | Desecrate offers wrong mods or violates a Bone level floor. |
...
| `data/crafting/runtime-registry.json` | Hand-authored tab metadata and 37 foundational runtime definitions: 33 supported, three blocked for missing data, and one probability-unverified. Generated Essence, Bone, and socketable families expand this without display-name matching. | Changing a verified visible craft's presentation or declarative runtime binding. |
| `data/crafting/currency-index.json` | **Generated authoritative registry.** It audits all 530 retained source records plus runtime-only Hinekora's Lock: 531 unique definitions, ten tabs, 422 runtime-bound definitions, 415 supported startup controls, and 522 visible catalogue definitions, with exact blockers, evidence, fixtures, applicability, and handlers. | Adding or auditing a crafting item. Generate it with `tools/build-currency-index.mjs`. |
...
- All nine Gnawed, Preserved, and Ancient equipment Bones execute through the shared Well flow with exact retained family, rarity, and item-level restrictions; these shared transitions remain labelled inferred. Ancient variants apply Minimum Modifier Level 40 to ordinary and Desecrated choices using the normalized 32 Jewel-at-level-1 / 193 equipment-at-level-65 split. Preserved Vertebrae remains blocked because the workbench has no Waystone model.
- Unknown Abyssal Bones and conflicting directional Necromancy Omens fail atomically. Omen of Light accepts revealed or unrevealed eligible Desecrated modifiers and is consumed once on the successful guarded Annulment path, including Hinekora foresight. The UI uses the engine-returned Echoes reroll count. Target-version Well weighting/localization and Lich guarantees remain explicit parity limits.

### Task 06 Sockets, Runes, and Soul Cores checkpoint

- Item state carries deterministic typed sockets with atomic malformed-state rejection, save/load compatibility, Undo/Redo, and a dedicated tooltip section. The runtime explicitly infers each concrete base's source `socketCount` as its maximum and starts fresh items with zero added sockets.
- Artificer's Orb adds one empty socket up to that inferred cap. The registry exposes 284 current Runes, Soul Cores, Idols, Abyssal Eyes, and Congealed Mist records; insertion uses exact retained item-class/family stat records, ranges, limits, and corruption flags.
- Three Talisman-only augments remain blocked because no selectable Talisman base exists, eight records remain deprecated, and replacement/extraction remain unavailable because no retained method defines those transitions.
